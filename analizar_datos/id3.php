...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['tabla'])) {
    ...
    // Consulta para obtener las claves primarias de la tabla seleccionada
    $query_ids = "SHOW KEYS FROM $tabla_seleccionada WHERE Key_name = 'PRIMARY'";
    $result_ids = $conn->query($query_ids);

    if ($result_ids) {
        if ($result_ids->num_rows > 0) {
            // Mostrar las claves primarias como opciones en el select
            while ($row_id = $result_ids->fetch_assoc()) {
                echo "<option value='{$row_id['Column_name']}'>{$row_id['Column_name']}</option>";
            }
        } else {
            echo "<option value=''>No se encontraron claves primarias en la tabla seleccionada</option>";
        }
    } else {
        echo "<option value=''>Error al obtener las claves primarias: " . $conn->error . "</option>";
    }
