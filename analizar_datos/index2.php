    ...
    while ($row = $tables_result->fetch_assoc()) {
        ...
        if ($columns_result->num_rows > 0) {
            while ($column_row = $columns_result->fetch_assoc()) {
                $table_info['columnas'][] = $column_row["Field"];
            }
        } else {
...
foreach ($database_info as $table_name => $table_info) {
    ...
    echo "Columnas:<br>";
    foreach ($table_info['columnas'] as $column_name) {
        echo "- $column_name<br>";
    }
    echo "Datos:<br>";
...
foreach ($database_info as $table_name => $table_info) {
    ...
    echo "<h3>Columnas:</h3>";
    echo "<table>";
    echo "<tr><th>Nombre de la Columna</th><th>Tipo de Dato</th><th>Clave Primaria</th><th>Clave Foránea</th></tr>";
    foreach ($table_info['columnas'] as $column_name => $column_info) {
        echo "<tr>";
        echo "<td>$column_name</td>";
        echo "<td>{$column_info['type']}</td>";
        echo "<td>" . ($column_info['is_primary'] ? "Sí" : "No") . "</td>";
        echo "<td>" . ($column_info['is_foreign'] ? "Sí" : "No") . "</td>";
        echo "</tr>";
    }
    echo "</table>";
