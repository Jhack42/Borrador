...
if ($result_docente_bienvenida->num_rows > 0) {
    while ($row_docente_bienvenida = $result_docente_bienvenida->fetch_assoc()) {
        ...
        if ($result_evaluaciones->num_rows > 0) {
            ...
            $query_evaluaciones2 = "SELECT * FROM $tabla3 WHERE $claves_foranea3 = $valor2";
            $result_evaluaciones2 = $conn->query($query_evaluaciones2);

            if ($result_evaluaciones2->num_rows > 0) {
                echo "<h3>Análisis de la segunda tabla $tabla3 relacionado con el $claves_foranea3 $valor2:</h3>";
                echo "<table border='1'>";
                while ($row_evaluacion2 = $result_evaluaciones2->fetch_assoc()) {
                    echo "<tr>";
                    foreach ($row_evaluacion2 as $key => $value) {
                        echo "<td>$key: $value</td>";
                        // Aquí podrías acceder al valor del FOREIGN KEY y realizar una consulta adicional
                        if ($key === $foreign_key_columna2) {
                            $foreign_key_value = $value;
                            // Realizar una consulta adicional utilizando el valor del FOREIGN KEY
                            $query_foreign_key_info = "SELECT * FROM tabla_relacionada WHERE columna_foreign_key = $foreign_key_value";
                            $result_foreign_key_info = $conn->query($query_foreign_key_info);
                            if ($result_foreign_key_info->num_rows > 0) {
                                while ($row_foreign_key_info = $result_foreign_key_info->fetch_assoc()) {
                                    // Aquí puedes imprimir o manipular los resultados de la consulta adicional
                                }
                            }
                        }
                    }
                ...
                echo "No se encontraron resultados para el $claves_foranea3 $valor2 en la segunda tabla $tabla3";
            }
